...
import com.bjxc.supervise.service.ProvinceCoachSuperviseService;
import com.bjxc.supervise.service.UpdateService;

@RestController
@RequestMapping("/province")
public class ProvinceCoachSuperviseAPI {
    private static final Logger logger = LoggerFactory.getLogger(ProvinceCoachSuperviseAPI.class);

    @Resource
    ProvinceCoachSuperviseService provinceCoachSuperviseService;
    @Resource
	...
	public Result coach(ServletRequest request ){
		...
		try
		{
			...
			result = provinceCoachSuperviseService.coach(params);
			/*if(result.getCode()==200){
				updateService.updateCoach(params);
			}*/
		} catch (Throwable ex) {
			result.error(ex);
