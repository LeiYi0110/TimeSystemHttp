...
import com.bjxc.supervise.service.ProvinceStudentSuperviseService;
import com.bjxc.supervise.service.UpdateService;

@RestController
@RequestMapping("/province")
public class ProvinceStudentSuperviseAPI {
    private static final Logger logger = LoggerFactory.getLogger(ProvinceStudentSuperviseAPI.class);

    @Resource
    ProvinceStudentSuperviseService provinceStudentSuperviseService;
	...
	public Result student(ServletRequest request ){
		...
		try
		{
			...
			result = provinceStudentSuperviseService.student(params);
			/*if(result.getCode()==200){
				try {
					updateService.updateStudent(params);
				} catch (Exception e) {
			...
			result.error(ex);
			logger.error("ProvinceStudent api",ex);
		}
		return result;
	}
}
